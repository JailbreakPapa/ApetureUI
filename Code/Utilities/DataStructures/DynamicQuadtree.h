...
class WD_UTILITIES_DLL wdDynamicQuadtree
{
...
public:
  ...
  void FindVisibleObjects(const wdFrustum& viewfrustum, WD_VISIBLE_OBJ_CALLBACK callback, void* pPassThrough = nullptr) const;
...
private:
  ...
  bool InsertObject(const wdVec3& vCenter, const wdVec3& vHalfExtents, const wdDynamicTree::wdObjectData& Obj, float minx, float maxx, float minz,
    float maxz, wdUInt32 uiNodeID, wdUInt32 uiAddID, wdUInt32 uiSubAddID, wdDynamicTreeObject* out_Object);

  /// \brief Recursively checks which nodes are visible and calls the callback for each object at those nodes.
  void FindVisibleObjects(const wdFrustum& Viewfrustum, WD_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx, float minz,
    float maxz, wdUInt32 uiNodeID, wdUInt32 uiAddID, wdUInt32 uiSubAddID, wdUInt32 uiNextNodeID) const;

  /// \brief Recursively checks in which node a point is located and calls the callback for all objects at those nodes.
  bool FindObjectsInRange(const wdVec3& vPoint, WD_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx, float minz, float maxz,
    wdUInt32 uiNodeID, wdUInt32 uiAddID, wdUInt32 uiSubAddID, wdUInt32 uiNextNodeID) const;
